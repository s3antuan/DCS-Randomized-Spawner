...
--
-- Change log:
-- v1.1: small changes to function optional arguments.
-- v1.2: manual spawn from menu won't consider limit.
--
...
do
	...
	function RepeatingSpawner.new(init_name, init_routeTable, init_templateTable)
		...
		local function spawnAI(manual)
			manual = manual or false
			if not manual then 
				if limit > 0 and scheduled_count >= limit then
					return
				else
		...
		function self.getScheduledCount()
		...
		function self.spawnOneGroup(manual)
			spawnAI(manual)
		end
		...
		function self.run()
			if controlGroupName then
				...
				MENU_GROUP_COMMAND:New(menuGroup, "Spawn One Group Immediately", menu, self.spawnOneGroup, true)
				for i = 1, 8 do
				...
				MENU_COALITION_COMMAND:New(menuSide, "Spawn One Group Immediately", menu, self.spawnOneGroup, true)
				for i = 1, 8 do
					if scheduleTable[i] then
	...
	function MenuShowRepeatingSpawnerStatus(repeatingSpawnerTable, menuText)
		menuText = menuText or "Show Repeating Spawner Status"
		local function getSpawnerStatus()
			...
			for _, spawner in pairs(repeatingSpawnerTable) do
				if type(spawner) == "table" then
					...
					local scheduled_count = spawner.getScheduledCount()
					local limit = spawner.getLimit()
					if subMenu then
						message = message .. subMenu .. "/"
					end
					message = message .. string.format("%s: Lv. %d ", name, level)
					if limit > 0 then
						message = message .. string.format("[%2d(%2d)/%2d spawned]\n", count, scheduled_count, limit)
					else
						message = message .. string.format("[%2d spawned]\n", count)
					end
		...
		MENU_MISSION_COMMAND:New(menuText, nil, function()
